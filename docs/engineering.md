...
以前的项目结构比较看写代码的人的喜好，虽然一般部门里都有 “团队规范” 这种东西，但靠自觉性去配合的事情，还是比较难做到统一，特别是项目很赶的时候。
...
虽然也可以自己改成别的，但我多年工作实际接触下来，以及从很多开源项目的代码里看到的，都是沿用脚手架创建的项目结构（不同脚手架创建的结构会有所不同，但基于同一技术栈的项目基本上都具备相同的结构）。
...
不管你是接手过其他人的代码，或者是修改自己不同时期的代码，可能都会遇到这样的情况，例如一个模板语句，上面包含了很多属性，有的人喜欢写成一行，属性多了维护起来很眼花：
...
传统的页面没有跨域支持，如果服务端的跨域设置没有配置好，或者仅允许线上域名访问，那么本地开发非常辛苦，同样的，写好页面要部署了，也得先找服务端同学去操作，有问题还得再找人家处理。

工程化项目可以借助构建工具实现本地代理，不管服务端有没有允许跨域，本地开发都可以进行代理访问无任何限制，部署之前也可以自己先通过脚手架提供的本地生产服务预览构建出来的页面，如果有问题，可以立即修改后重新打包再预览，减少沟通成本。
