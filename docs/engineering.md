...
在了解了 MPA 与 SPA 之后，先了解另外两个有相关联的名词： CSR 与 SSR ，同样的，这一对也是代表着两个完全相反的开发模式和用户体验，它们的全称和中文含义如下：
...
如果已经阅读过 [MPA 与 SPA](#mpa-与-spa) ，那么应该很容易就联想到： CSR 与 SPA 、 SSR 与 MPA 之间是不是有某种联系？答案是肯定的！

SPA 单页面应用正是基于 CSR 客户端渲染实现的，这是一种利用 AJAX 技术，把渲染工作从服务端转移到客户端完成，不仅客户端的用户体验更好，前后端分离的开发模式更加高效。

但随之而来的首屏加载较慢、不利于 SEO 优化等缺点，也是无法避免的问题。

:::tip
如果还不了解 CSR 的概念、实现原理和优缺点的话，建议先阅读 [MPA 与 SPA](#mpa-与-spa) 里的 [单页面应用](#单页面应用) 部分。
:::

因此结合 MPA 的优点和 SPA 的优点，一种新的技术随之诞生，这就是 SSR 服务端渲染。
