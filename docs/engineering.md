...
在上一节，我们提到了前端工程化 [在求职竞争上的优势](#求职竞争上的优势) ，里面列出的招聘要求例子都提及到了 Vue 和 React 这些主流的前端框架，前端框架是前端工程化开发里面不可或缺的成员。

框架能够充分的利用前端工程化相关的领先技术，不仅在开发层面降低开发者的上手难度和提升开发效率，在构建出来的项目成果上也有着远比传统开发更优秀的用户体验。

本书会结合 Vue.js 框架，从项目开发的角度，在帮助开发者入门前端工程化的同时，更快速的掌握一个流行框架的学习和使用。
