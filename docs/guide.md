...
每个 Current 版本会持续 6 个月的时间，期间会被活跃的维护和变更， 6 个月后：

大版本号是奇数的（ e.g. v17.x.x ）将变为不支持状态，不会进入 LTS 版本。
   
大版本号是偶数的（ e.g. v16.x.x ）会按照发布节点进入 LTS ，并且作为活跃状态投入使用。
...
:::tip
关于 Node 的版本发布情况可以在官网查看 [长期维护版（ LTS ）日程](https://nodejs.org/zh-cn/about/releases/) 。
:::
