...
早期的部分内容使用了 @vue/cli 创建的项目做演示，后期因为 Vite 开始逐渐作为 Vue 项目的主流构建工具，因此后面的新增内容使用了 Vite 创建的项目，造成了上下文存在一些演示内容不统一的情况，本次对内容进行了检查和合并，移除了涉及到 @vue/cli 项目的旧内容，统一更新为使用 Vite 项目讲解。
